...
将目标网卡（按「子网包含 `192.168.73.1`」识别，即本机 IP 与目标同网段）对端设备的 Web 配置页面转发到本机端口，供外部（如局域网内的 PC）访问。

### 使用场景

安卓设备通过以太网卡（按「子网包含 `192.168.73.1`」识别，即本机 IP 与目标同网段）连接到另一台设备（如树莓派等单板计算机），对端 IP 固定为 `192.168.73.1`，在其 80 端口提供 Web 配置页面。端口转发功能将安卓本机的某个端口映射到对端的 80 端口，外部设备只需访问 `http://<安卓设备IP>:8000` 即可打开对端的 Web 配置页面，无需直接连接对端设备的网络。

### 技术实现

Linux 上通常用 `socat` + `SO_BINDTODEVICE` 实现，但 `SO_BINDTODEVICE` 需要 root 权限，普通 Android 应用无法使用。本实现使用等价的非 root 方案：

- 先按「子网包含 `192.168.73.1`」在 `NetworkInterface` 枚举中识别目标网卡（本机 IP 与目标同网段，如本机 `192.168.73.2`），再通过 `ConnectivityManager` 找到同名的 `Network`，对上游 Socket 调用 `Network.bindSocket()`，强制流量走该网卡。
- 若该网卡仅作为裸网卡存在（未被系统登记为 Network），则回退到将 Socket 源地址绑定到该网卡的 IPv4 地址，Linux 内核依据源地址选择出口网卡。

### 架构

| 组件 | 说明 |
|------|------|
| `PortForwarder` | 转发引擎：本机 TCP Server 接收连接 → 上游 Socket 走目标网卡连接 `192.168.73.1:80` → 双向数据拷贝 |
| `PortForwardService` | 独立前台服务，管理 `PortForwarder` 生命周期，与 `PointerService` 解耦，可单独启停 |
| `PortForwardScreen` | Compose 设置界面，配置监听端口、查看转发状态 |

### 热插拔支持

- 目标网卡存在时正常转发。
- 目标网卡拔出时暂停上游连接（监听端口保留），等待网卡恢复。
- 目标网卡重新插入时自动恢复转发，无需手动重启。
- 检测机制：`NetworkCallback` + 2 秒轮询兜底。
...
|------|--------|------|
| 监听端口 | `8000` | 本机对外暴露的端口 |
| 上游地址 | `192.168.73.1:80` | 目标网卡对端设备的 Web 服务 |

监听端口可在应用的"端口转发" Tab 中修改。

### 使用方式

1. 打开应用，切换到**端口转发** Tab。
2. 确认/修改监听端口（默认 8000），点击**启动转发**。
3. 在局域网内的其他设备上访问 `http://<安卓设备IP>:8000` 即可访问对端 Web 配置页面。

## DEBUG
使用logcat查看日志：adb logcat | grep -i "vpointer\|PointerService\|MainActivity\|PortForward"
