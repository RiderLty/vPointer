...
将目标网卡（按 MAC `00:02:73:6a:96:01` 识别，名称可能为 eth0/eth1/eth2 等）对端设备的 Web 配置页面转发到本机端口，供外部（如局域网内的 PC）访问。

### 使用场景

安卓设备通过以太网卡（按 MAC `00:02:73:6a:96:01` 识别）连接到另一台设备（如树莓派等单板计算机），对端 IP 固定为 `192.168.73.1`，在其 80 端口提供 Web 配置页面。端口转发功能将安卓本机的某个端口映射到对端的 80 端口，外部设备只需访问 `http://<安卓设备IP>:8000` 即可打开对端的 Web 配置页面，无需直接连接对端设备的网络。

### 技术实现

Linux 上通常用 `socat` + `SO_BINDTODEVICE` 实现，但 `SO_BINDTODEVICE` 需要 root 权限，普通 Android 应用无法使用。本实现使用等价的非 root 方案：

- 先按 MAC `00:02:73:6a:96:01` 在 `NetworkInterface` 枚举中识别目标网卡，再通过 `ConnectivityManager` 找到同名的 `Network`，对上游 Socket 调用 `Network.bindSocket()`，强制流量走该网卡。
