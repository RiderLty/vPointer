...
将 USB 网卡（usb0）对端设备的 Web 配置页面转发到本机端口，供外部（如局域网内的 PC）访问。

### 使用场景

安卓设备通过 USB 网卡连接到另一台设备（如树莓派等单板计算机），对端 IP 固定为 `192.168.73.1`，在其 80 端口提供 Web 配置页面。端口转发功能将安卓本机的某个端口映射到对端的 80 端口，外部设备只需访问 `http://<安卓设备IP>:8000` 即可打开对端的 Web 配置页面，无需直接连接对端设备的网络。

### 技术实现

Linux 上通常用 `socat` + `SO_BINDTODEVICE` 实现，但 `SO_BINDTODEVICE` 需要 root 权限，普通 Android 应用无法使用。本实现使用等价的非 root 方案：

- 通过 `ConnectivityManager` 找到 usb0 网卡对应的 `Network`，对上游 Socket 调用 `Network.bindSocket()`，强制流量走 usb0 网卡。
- 若 usb 仅作为裸网卡存在（未被系统登记为 Network），则回退到将 Socket 源地址绑定到该网卡的 IPv4 地址，Linux 内核依据源地址选择出口网卡。

### 架构

| 组件 | 说明 |
|------|------|
| `PortForwarder` | 转发引擎：本机 TCP Server 接收连接 → 上游 Socket 走 usb0 连接 `192.168.73.1:80` → 双向数据拷贝 |
...
- usb0 网卡存在时正常转发。
- usb0 拔出时暂停上游连接（监听端口保留），等待网卡恢复。
- usb0 重新插入时自动恢复转发，无需手动重启。
...
|------|--------|------|
| 监听端口 | `8000` | 本机对外暴露的端口 |
| 上游地址 | `192.168.73.1:80` | usb0 对端设备的 Web 服务 |
